...
4. **填隐私 frontmatter**：title / slug / sidebar_label / audience / era / technique / attack_surface / severity / maturity / evidence / tags / sources。`sources` **至少两条可核查一手出处**；任何量化声明必回一手。叙事长标题必配短 `sidebar_label`（名词短语）。`evidence` 取 sources 里最强一类作**主标签**；混合证据**不另设字段**，在「延伸阅读」逐条点明、必要时开头写「主要：X；补充：Y」（见 terminology.md「evidence」）。
