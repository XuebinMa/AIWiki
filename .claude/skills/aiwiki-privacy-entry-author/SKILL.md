...
4. **填隐私 frontmatter**：title / slug / sidebar_label / audience / era / technique / attack_surface / severity / maturity / evidence / tags / sources。`sources` **至少两条可核查一手出处**；任何量化声明必回一手。**来源优先级：业界实践 > 学术**——厂商安全 / 数据治理文档、真实部署、隐私事故复盘优先（论文降为机制背书，见 `STYLE-GUIDE.md`「来源优先级」）；但本主题攻防机制多源于论文，**三道硬闸门不松**。叙事长标题必配短 `sidebar_label`（名词短语）。`evidence` 取 sources 里最强一类作**主标签**；混合证据**不另设字段**，在「延伸阅读」逐条点明、必要时开头写「主要：X；补充：Y」（见 terminology.md「evidence」）。
5. **写九节**（顺序不可变）：一句话摘要 → 机制（我这边发生了什么，第一人称 + 机制图）→ 威胁面 → 防护原理 → 落地实现（配方 + 结尾收一个 **最小可测试断言** 小块：怎么测 / 通过证明什么 / 失败看到什么——把风险变成 CI / eval / 审计里可回归的断言）→ 真实案例 / 生产部署（**标题可按条目类型微调、须与 maturity 一致**：真实攻击演示 / 研究进展·工程可行性 / 相邻事故 / 厂商现状——别给试验级条目挂「生产部署」）→ 残余风险与权衡 → 版本说明（`:::note`）→ 延伸阅读与出处。可选小节：合规映射 / 何时这条防护不适用 / 与相邻技术的区别 / 框架差异（短、打版本戳，无内容就删）。
