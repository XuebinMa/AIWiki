...
- **预印本数字会随版本漂**：同一 arXiv 预印本常多版（v1/v2/v3），标题 / 数字都可能变；两次独立核验出不同值时（本轮 AgentLeak：内漏 74% vs 68.8%），**别锚死任一值**——稳健定性结论照写，绝对值用**区间 + 「随版本 / 口径浮动」**标注（例：内漏约 2.5–2.6× 对外、约 69%–74% vs 27%–28%）。**venue 也要核准**：workshop ≠ 主会（MAGPIE 曾被误当 NeurIPS 主会，实为 ResponsibleFM workshop），预印 / workshop 一律 ⚠️。
...
- **三种处置，别只会「新立条」**：① 角度 / 卷 / 根因确不同 → **新立**；② 同根因 / 同 technique，或只有**单篇薄预印**撑 → **并入**现有条（作威胁面升级、或延伸阅读里的 ⚠️ 方向性指针，别单列成条）；③ 证据不合「误区 / 假安全」形（如某手段其实**降低**风险）→ **搁置**。**并入 / 搁置同样记 `BACKLOG-privacy.md`「并入 / 搁置」区**（附落点 + 一手源），别只留对话。（本轮示例：kv-cache→并 `inference-side-channels`、a2a→并 `multi-agent-internal-leakage`、compression→搁置。）
...
- **并行子代理可能限流断报**：子代理可能**文件已写完、却在交报告前撞上会话限流**（本轮 multimodal 条即此）——**别当它没干活**：先 `ls` / `wc -l` 确认文件在不在，在就照常质量过；**缺它的核验报告时，主控自己逐条复核其一手源**再放行。
- **主控独立复核，不照单全收（证据纪律对子代理一视同仁）**：子代理报「已核」≠ 真核——发布前主控 WebSearch 复核**承重脊梁源**（venue / year / 数字）。本轮据此改正：AgentLeak 数字版本出入、Apple AFM 3 年份（+ 删未核实的参数）、MAGPIE workshop 归属、MCP「最小采集」引文归属到 Anthropic 政策（非 MCP 规范本身）。红线格式也复核：第一人称红线必须**散文**，不用 `:::danger`、不用 ❌/✅ 列表（本轮修过 2 处）。
