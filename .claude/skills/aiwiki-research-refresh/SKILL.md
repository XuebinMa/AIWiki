...
- **来源质量硬标准**（命门，宁缺毋滥）：
  - **优先（业界实践优先于学术，见 `STYLE-GUIDE.md`「来源优先级」）**：① **业界实践**——厂商工程博客与最佳实践（Anthropic / OpenAI / Google / Thoughtworks…）、事故复盘、可核查真实事故库（AI Incident Database、权威媒体如 Fortune）、从业者一手账（METR、Martin Fowler、Simon Willison），即「各家线上实际怎么做 / 怎么翻车」；② 再用 arXiv / 会议或期刊（IEEE、ACM、TACL…）实证作**机制背书**。论文不当头条。
