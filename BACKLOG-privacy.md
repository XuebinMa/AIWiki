...
| 每条加「最小可测试断言」块 | 🟡 | 强认同工程价值；但**倾向并入「落地实现」**小节而非新增第十节（防九节结构膨胀）。作模板+skill+ 回填四条 + 后续统一带的演进。见结构待办 |
...
- ⬜ **「最小可测试断言」模板块（R3）**：倾向**并入「落地实现」**小节（不新增第十节）；定稿后改 `entry-template` + `aiwiki-privacy-entry-author` skill + **回填已写四条** + 后续条目统一带（CI/eval/audit 怎么测、失败看到什么、通过证明什么）。
