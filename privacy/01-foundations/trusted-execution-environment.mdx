...
1. **内存 / 显存被硬件加密、隔离。** 我处理你 prompt 的那块内存，由芯片在运行时加密、加完整性保护；OS、hypervisor、同机其他租户即便有更高权限，读到的也是密文。机密计算联盟（CCC）把 TEE 的保证定义为三条：**数据机密性、数据完整性、代码完整性**（CCC, *A Technical Analysis of Confidential Computing*）。
...
- **TEE vs HE / MPC**：TEE 靠**硬件信任**换性能——快、通用，但要信芯片厂商、且有侧信道；HE / MPC 靠**密码学**——不信任何硬件，但慢得多（见《HE·MPC》——同卷另一条地基）。三者常被放在一起选型：用硬件信任换速度，还是用密码学换「零硬件信任」。
