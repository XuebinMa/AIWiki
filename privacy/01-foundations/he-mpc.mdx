---
title: 同态加密与安全多方计算（HE·MPC）：不信任何硬件地在密文上算，代价是慢一大截
slug: he-mpc
...
安全不来自「藏在某个硬件里」，而来自**密码学假设**：HE 靠格上的困难问题 + bootstrapping 控噪；MPC 靠秘密分享 / 混淆电路 + 通信轮次，并在**明确的威胁模型与合谋阈值**下证明「除输出外不泄露任何信息」。承重点正在于此——**这是与 TEE 正交的信任模型**：TEE 要你信芯片厂商，HE / MPC 要你信数学假设与（MPC 的）合谋阈值，不信任何硬件。
...
4. 定安全参数：λ ≥ 128 位；写明威胁模型与合谋假设，别留默认。
...
- 怎么测：① 功能——密文 / 多方算出的结果与明文直算对比（HE 近似方案 CKKS 要带**误差界**）；② 安全——核 λ ≥ 128 位、威胁模型与合谋阈值是否写明并与协议匹配。
...
- **HE·MPC vs TEE**：同是「把数据交出去算却不泄露」，但信任模型相反——TEE 靠**硬件信任**（快、通用，但信芯片厂商 + 有侧信道，见《[可信执行环境](./trusted-execution-environment.mdx)》）；HE / MPC 靠**密码学**（零硬件信任，但慢一大截）。选型就是拿「速度」换「零硬件信任」，或反过来。
