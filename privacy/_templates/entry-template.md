...
evidence: 研究支持            # 证据主标签，取 sources 里最强一类；混合证据时补充类型在「延伸阅读」逐条点明（见 terminology.md「evidence」）
...
**最小可测试断言**（把配方收成可回归的检查，别停在「我们做了 X」）：

- 怎么测：本条风险如何在 CI / eval / 审计里被检测（给出探针 / 指标 / 工件）。
- 通过：通过时应证明什么（可核的指标或断言——阈值、可复算、断言全绿…）。
- 失败：失败时应看到什么、回去改哪一层。
...
> **本小节标题可按条目类型微调**（须与 `maturity` 一致、别名不实）：真实攻击演示 / 研究进展·工程可行性（`maturity=试验·研究`）/ 相邻事故 / 厂商现状——别给试验级条目挂「生产部署」这种过满标题。
...
混合证据时可在此开头补一句「主要：X；补充：Y」（见 terminology.md「evidence」）。

- [来源标题](https://...) —— 一句话点明它支撑了上文哪个论断（必要时点出该来源的证据类型）
