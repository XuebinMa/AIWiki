...
sources:
  - { title: "Apple Security Research, Private Cloud Compute: A new frontier for AI privacy in the cloud（官方：设备级远程证明 + 可验证透明日志 + 无特权访问）", url: "https://security.apple.com/blog/private-cloud-compute/" }
...
- **Apple Private Cloud Compute（PCC）**：为 Apple Intelligence 的云端请求设计——设备级远程证明、把生产软件镜像公开上**可验证透明日志**、**无特权运行角色**（连 Apple 自身的运维也不能特权访问用户数据），并开放部分关键组件源码供安全研究（Apple Security Research, *Private Cloud Compute*）。它是「把验证交给用户设备自动做」的范例。
- **NVIDIA H100 / H200 机密计算**：GPU TEE，标准 PyTorch / TensorFlow / ONNX 模型**不改**即可在飞地里跑，客户端加一步证明验证（NVIDIA 官方）。
...
:::note 适用版本
机密推理的**架构思路**（TEE + 远程证明 + 可验证透明）相对稳定，但**具体产品与硬件**演进很快：PCC、NVIDIA 机密计算、各云厂商的机密 VM / GPU 都在迭代，密码学路线的可行性也随硬件加速变化。本条按 2026-06 的产品现状表述（Apple PCC、NVIDIA H100/H200 机密计算）；具体能力、开销与威胁模型以厂商当下文档和你的实测为准。（出处核验于 2026-06。）
:::

## 延伸阅读与出处

证据为混合——**主要：官方文档**（Apple、NVIDIA）；**补充：标准**（CCC）+ 机制见卷一两条地基。

- [Private Cloud Compute: A new frontier for AI privacy in the cloud（Apple Security Research）](https://security.apple.com/blog/private-cloud-compute/) —— 官方：设备级远程证明 + 可验证透明日志 + 无特权访问 + 部分源码可审。
- [Confidential Computing on H100 GPUs（NVIDIA 官方）](https://developer.nvidia.com/blog/confidential-computing-on-h100-gpus-for-secure-and-trustworthy-ai/) —— 官方：GPU TEE，标准模型不改即可在飞地里跑 + 客户端证明。
