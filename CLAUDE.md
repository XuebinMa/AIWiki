...
- **复用分支铁律（防 squash 冲突）**：本仓库用 **squash 合并**——PR 合并后 `main` 上是一个**全新 SHA**，与你分支里的原始提交血缘不同。若合并后**不重置**就继续在同一条工作分支上提交，分支会同时压着「已被 squash 进 main 的原始提交」+ 新提交，git 把同一份改动当成两份重叠新增 → 下个 PR 必然冲突。**每次合并后、开始新工作前，先 `git fetch origin main && git reset --hard origin/main` 把工作分支对齐到最新 `main`**。（分支保护禁止 force-push，已踩过：只能改用「把 main 合并进分支」的普通合并提交来补救。）
