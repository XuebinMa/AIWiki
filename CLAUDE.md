...
- **主题二「LLM 隐私保护」（`privacy/`）**：面向工程师的 LLM 隐私攻防手册（机制 + 配方 + 案例，按 LLM 发展史分卷）。**已 44 条中英条目（六卷齐）+ 四条导读轴（必读 12 / 最小基线 / 事故索引 / 机制索引）+ 隐私地图与厂商清单工件**——立项 / 决策见 `PROPOSAL-privacy-book.md`，看板见 `BACKLOG-privacy.md`，专属规范见 `STYLE-GUIDE.md`「LLM 隐私保护主题专属规范」。**写隐私内容前必读这三处**；准确门槛更高（三道硬闸门 + 第一人称红线）。
...
- 写 / 改一条「AI 编码误区」条目：`aiwiki-entry-author`（七段结构，docs/）
- 写 / 改一条「LLM 隐私保护」条目：`aiwiki-privacy-entry-author`（九节结构 + 第一人称红线 + 三道硬闸门，privacy/）
- 出对应英文版（两主题通用）：`aiwiki-translator`
- 定期研究扩充新选题（去重 + 质量把关 + 走流水线）：「AI 编码误区」用 `aiwiki-research-refresh`；「LLM 隐私保护」用 `aiwiki-research-refresh-privacy`（去重基线 `privacy/`、三道硬闸门、一手出处、密码学存疑即 `:::caution`）。均适合月度定时触发器或手动跑。
...
- `npm run check:mirror-deep` —— 两实例镜像**结构对等**（## 标题数 / admonition 序列 / 内链目标集 / slug），防「中文改了英文没跟」的静默漂移（密度差刻意容忍，比对集合不比计数）
- `npm run check:counts` —— 散文里的**手写条目计数**（core-20 中英、本文件「现状」段）与实际条目数一致；新增手写计数处须登记进 `scripts/check-counts.js`（防 56→66→77 式漂移复发）
...
已铺 79 条误区，中英镜像齐（00 准备 ×17、01 灵感 ×6、02 需求 ×6、03 概要 ×7、04 详设 ×7、05 编码 ×15、06 测试 ×11、07 验收 ×10；06 含 AI 代码审查子簇 4 条，00/07 含 AI 治理 + 基础设施安全子簇 6 条，00 另有 1 条跨工具配置提权 CVE-2026-35603 类）+ 案例库 5 例 + 模板与清单 12 件（`toolkit`：5 基础 + 4 工作流 + 3 治理模板）+ 四条浏览/导读轴索引页：工具矩阵（`/tool-matrix`）、机制索引（`/mechanisms`，7 个根因桶覆盖全 79 条）、安全索引（`/threat-model`，按威胁维度聚合安全/治理条目与案例，顶部含「最小安全基线」8 条速用清单）、必读 20 条（`/core-20`）+ AI 代码审查专题页（`/code-review`，聚合审查子簇 4 条 + 分工工件）。**首个第二工具 Gemini CLI 试点已落地**：8 条主条目带「工具差异」小节、1 条 Gemini 独有条目（folder-trust 继承）、1 例 Tracebit RCE 案例。每条带证据类型标签（`terminology.md`「evidence」），可按阶段（侧边栏）、角色（`/roles`）、工具（`/tool-matrix`、`/tags/gemini-cli`）或机制（`/mechanisms`）浏览。具体写过什么以 `docs/` 实际内容为准。
