...
1. **An embedding is not a one-way hash.** It retains enough lexical and semantic information to be **inverted back to approximate original text** by a learned decoder — Morris et al.'s vec2text iteratively approaches the target vector and reconstructs sentences with high fidelity, recovering **real names** from embeddings of clinical notes (Morris et al., EMNLP 2023). So "we only stored vectors, not the text" is **not** anonymization.
...
4. Isolation regression tests: inject cross-tenant/cross-user probe documents,
   automatically test "can A's query recall B's chunks"; make it a CI gate, not
   a one-off manual spot check.
