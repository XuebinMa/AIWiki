...
- ⚠️ (single preprint, directional corroboration only) [Improving Google A2A Protocol: Protecting Sensitive Data and Mitigating Unintended Harms in Multi-Agent Systems (arXiv 2505.12490)](https://arxiv.org/abs/2505.12490) — **the adjacent gap at the protocol layer**: it analyzes Google A2A (the cross-vendor agent↔agent protocol) handling highly sensitive data like payment credentials / identity documents, and finds insufficient token-lifetime control, overbroad access scopes, and missing consent flows — proposing consent orchestration + ephemeral scoped tokens. Complementary to this entry: this entry measures "how much the model leaks over internal channels," it covers "**what controls the protocol layer lacks**"; for the MCP version of protocol-layer consent / tokens see [MCP data flow and least collection](./mcp-data-flow-privacy.mdx). ⚠️ Single preprint; its leakage-rate numbers are bound to its tested setup, not an independent conclusion.
