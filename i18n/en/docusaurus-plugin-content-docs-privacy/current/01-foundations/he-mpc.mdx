---
title: "Homomorphic encryption and secure multi-party computation (HE·MPC): compute on ciphertext with zero hardware trust — at a steep speed cost"
slug: he-mpc
...
4. Set security parameters: λ >= 128 bits; spell out the threat model and
...
- How to test: ① functionality — compare ciphertext / multi-party results against plaintext computation (HE approximate schemes like CKKS must carry an **error bound**); ② security — check λ >= 128 bits and that the threat model and collusion threshold are stated and match the protocol.
