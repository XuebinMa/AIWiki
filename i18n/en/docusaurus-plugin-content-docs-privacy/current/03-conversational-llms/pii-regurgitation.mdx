...
sources:
  - { title: "Microsoft Presidio FAQ — \"no guarantee that Presidio will find all sensitive information\" (official docs)", url: "https://microsoft.github.io/presidio/faq/" }
  - { title: "Microsoft Presidio — Supported entities (NER + rules + checksum detection methods, official docs)", url: "https://microsoft.github.io/presidio/supported_entities/" }
  - { title: "Google Cloud Sensitive Data Protection — InfoTypes and infoType detectors (official docs)", url: "https://docs.cloud.google.com/sensitive-data-protection/docs/concepts-infotypes" }
  - { title: "Google Cloud Sensitive Data Protection — De-identification transformation reference (masking / tokenization / format-preserving encryption, official docs)", url: "https://docs.cloud.google.com/sensitive-data-protection/docs/transformations-reference" }
  - { title: "Amazon Comprehend Detect PII — AWS AI Service Card (precision/recall trade-off + human review for high-risk, official docs)", url: "https://docs.aws.amazon.com/ai/responsible-ai/comprehend-detectpii/overview.html" }
  - { title: "Amazon Comprehend — Detecting PII entities (entity types and method, official docs)", url: "https://docs.aws.amazon.com/comprehend/latest/dg/how-pii.html" }
  - { title: "Azure AI Language — PII detection transparency note (false negatives can cause PII leakage + human review recommended, official docs)", url: "https://learn.microsoft.com/en-us/azure/ai-foundry/responsible-ai/language-service/transparency-note-personally-identifiable-information" }
...
<PrivacyMeta era="Volume 3 · Conversational LLMs" technique="PII detection, redaction & synthetic data" audience={['Privacy Engineer', 'ML Engineer', 'Compliance Engineer']} severity="High" maturity="Research" evidence="Official docs" />

> In one sentence: feed a corpus containing personal data into training, and I may reproduce it **in ordinary conversation** — not necessarily via a carefully crafted attack. In production, scrubbing is usually done with mature tooling (Microsoft Presidio / Google Cloud Sensitive Data Protection / AWS Comprehend + Macie / Azure AI Language PII) doing named-entity recognition (NER) + pattern detection, which **substantially lowers** regurgitation; but **all of these vendors' own docs state that detection is not guaranteed to find everything** — NER misses entities, and PII can be **reconstructed across fields** (mechanism evidence: Lukas et al., IEEE S&P 2023). Conclusion first: treat PII regurgitation as "**reduce + audit + accept it can't be fully eliminated**," and don't read "we ran scrubbing" as "no PII" — that's the most common false security here.
...
2. Scrub before training: use a mature PII tool (self-hosted Microsoft Presidio, or
   managed Google Cloud Sensitive Data Protection / AWS Comprehend + Macie / Azure AI
   Language PII) + domain rules for NER de-identification, but treat it as "reduce,"
   not "eliminate"; record the miss rate / which entity types are covered — don't
   assume 100% (every vendor's own docs say detection isn't guaranteed to be complete).
...
## Real-world deployment / research status (engineering feasibility)

### What the industry actually uses: production PII detection / scrubbing tools (read this layer first)

In production, teams don't hand-roll PII scrubbing for corpora / outputs — they reach for one of these mature tools. The **mechanisms are broadly the same** — named-entity recognition (NER) + regex/dictionary + checksums, plus extensible custom recognizers; what differs is managed vs. self-hosted, the entity types covered, and the accompanying de-identification transforms. The key point: **every vendor's own documentation states that detection is not guaranteed to catch everything** — which is the vendor-level proof of this entry's "ran scrubbing = no PII" false security.

- **Microsoft Presidio** (open-source, self-hosted): a hybrid design — spaCy NER + regex + rules + checksums + custom recognizers for detection, with an Anonymizer applying replace / mask / redact operators. The official FAQ says plainly: **"there is no guarantee that Presidio will find all sensitive information,"** and recommends that "additional systems and protections should be employed." Use it as a "reduce" tool, not a boundary.
- **Google Cloud Sensitive Data Protection** (formerly Cloud DLP): 200+ built-in infoType detectors (dictionary / regex / contextual rules, plus custom), with de-identification transforms covering redaction / masking / bucketing / date-shift / tokenization / format-preserving encryption. The docs state that built-in infoTypes **"are not a perfectly accurate detection method and can't guarantee compliance,"** that "you must decide what data is sensitive," and that you should test your configuration.
- **AWS: Amazon Comprehend Detect PII + Amazon Macie**: Comprehend uses an ML model to detect PII entities (~22 universal + 14 country-specific, English / Spanish); Macie uses managed data identifiers to scan PII in S3. AWS's **AI Service Card** states that performance varies by dataset and that you tune the confidence threshold (**high threshold favors precision, low favors recall** — the two trade off), requires that high-risk use cases **"incorporate human review,"** and recommends periodic retesting to catch drift.
- **Azure AI Language — PII detection**: a managed detection + redaction service. Its official **transparency note** spells it out: **"both false positive and false negative errors can occur,"** and **"false negatives could lead to personal information leakage,"** so it recommends **human review** in redaction workflows and stresses that context affects detection quality.

To close the loop: these are all **recall-bounded NER / pattern / ML detectors** — they **lower** regurgitation, they don't **zero it out**; and **de-identification ≠ de-association** (remove explicit identifiers and a combination of quasi-identifiers can still reconstruct a person). The research below explains the **mechanism** behind "why it reduces rather than eliminates."

### Why it reduces, doesn't eliminate: empirical evidence at the mechanism level (research backing)
...
"PII in the training corpus gets reproduced by the model in generation, and scrubbing reduces rather than eliminates it" is a **model-independent, paradigm-level** conclusion (the root cause is that an LM learns token co-occurrences and doesn't distinguish whether something is PII), common across vendors. The specific regurgitation rate and how much a given scrubber covers vary with model scale, corpus, and scrubbing pipeline; Lukas et al.'s experiments are tied to their setup (GPT-2-scale + Flair NER) and **don't transfer directly** to your model — you must re-measure with your own probes. The capabilities, entity coverage, and wording of Presidio / Google Sensitive Data Protection / AWS Comprehend + Macie / Azure PII shift with product iterations (vendors often rename / change quotas) — check each vendor's official docs for the latest terms before citing. Stamped 2026-06. (All sources verified 2026-06.)
:::

## Further reading and sources

**Primary: official docs for production PII tooling (each vendor states detection isn't guaranteed to be complete); supplementary: empirical research (the mechanism for "why it reduces rather than eliminates").**

- [Microsoft Presidio — FAQ](https://microsoft.github.io/presidio/faq/) and [Supported entities](https://microsoft.github.io/presidio/supported_entities/) — an open-source, self-hosted NER + rules + checksum scrubbing library; the FAQ states "no guarantee that Presidio will find all sensitive information." Usable in recipe step 2; audit its coverage and misses per this entry.
- [Google Cloud Sensitive Data Protection — InfoTypes and detectors](https://docs.cloud.google.com/sensitive-data-protection/docs/concepts-infotypes) and [De-identification transformation reference](https://docs.cloud.google.com/sensitive-data-protection/docs/transformations-reference) — 200+ built-in infoTypes + masking / tokenization / format-preserving encryption; the docs state built-in detectors "are not a perfectly accurate detection method and can't guarantee compliance."
- [Amazon Comprehend Detect PII — AWS AI Service Card](https://docs.aws.amazon.com/ai/responsible-ai/comprehend-detectpii/overview.html) and [Detecting PII entities](https://docs.aws.amazon.com/comprehend/latest/dg/how-pii.html) — ML entity detection; the docs state precision/recall trade off via the threshold, high-risk use cases need human review, and you should retest periodically for drift.
- [Azure AI Language — PII detection transparency note](https://learn.microsoft.com/en-us/azure/ai-foundry/responsible-ai/language-service/transparency-note-personally-identifiable-information) — the docs state "false negatives could lead to personal information leakage" and recommend human review.
- [Analyzing Leakage of Personally Identifiable Information in Language Models (Lukas et al., IEEE S&P 2023; arXiv 2302.00539)](https://arxiv.org/abs/2302.00539) — PII leakage taxonomy (extraction / reconstruction / inference) + quantified metrics + the empirical "scrubbing reduces, doesn't eliminate." Mechanism backing: explains why every tool above claims only "reduce."
