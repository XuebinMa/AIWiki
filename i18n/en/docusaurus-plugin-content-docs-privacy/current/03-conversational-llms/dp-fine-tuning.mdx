...
## Framework differences (DP libraries; stamped 2026-06, defer to each library's current docs)

The DP-SGD skeleton is the same across libraries, but the **engineering and default accounting** differ — check when porting:

- **Opacus (PyTorch)**: per-sample gradients via hooks / functorch, built-in RDP-style accounting; supports ghost clipping to save memory.
- **TensorFlow Privacy**: DP optimizers + several accountants; integrates with Keras training loops.
- **JAX side (e.g. jax-privacy)**: per-sample gradients come naturally via `vmap`, often cheaper on memory at large batch.

The differences are mainly in **how per-sample gradients are computed, which accountant is used, and whether ghost clipping is supported** — these affect memory / speed and whether your reported (ε, δ) can be independently recomputed, **not the formal DP definition**. When switching libraries, recompute ε under one convention; don't assume cross-library numbers are directly comparable. (This subsection ages fast; defer to each library's release docs.)
