...
sources:
  - { title: "OpenAI, Supervised fine-tuning guide (official: input moderation on fine-tuning training data + output-moderation eval on the resulting model; a usage-policy hit blocks the fine-tuning job / deployment, and you aren't charged)", url: "https://platform.openai.com/docs/guides/supervised-fine-tuning" }
  ...
  - { title: "Anthropic, API and data retention (official: commercial / API I/O not used for training by default; as of 2025-09-14 standard log retention cut from 30 to 7 days, 30-day opt-in via DPA, ZDR)", url: "https://platform.claude.com/docs/en/manage-claude/api-and-data-retention" }
  ...
  - { title: "Qi et al., Fine-tuning Aligned Language Models Compromises Safety, Even When Users Do Not Intend To! (ICLR 2024) (mechanism backing: fine-tuning erodes alignment)", url: "https://openreview.net/forum?id=hTEGyKf0dZ" }
---

import PrivacyMeta from '@site/src/components/PrivacyMeta';

<PrivacyMeta era="Volume 6 · Governance and compliance" technique="Inference-service privacy" audience={['Privacy Engineer', 'Compliance Engineer', 'ML Engineer']} severity="Medium" maturity="Research" evidence="Official docs" />
...
## A real case / vendor state (engineering feasibility)

Lead with **how the industry actually runs fine-tuning-as-a-service in production** — vendors' data-boundary terms, and how they actually guard fine-tuning on the wire; then put Qi et al.'s research underneath as the mechanism (why you must guard it, and why guarding still doesn't catch alignment erosion).

**How the industry does it, part 1 — how vendors guard fine-tuning on the wire (OpenAI official, stamped 2026-06)**: opening a fine-tuning API to users does not mean vendors leave it unattended — they put an **automated moderation gate at each end**, which is exactly a first-hand sample of "how the industry actually does it":

- **Input side: moderate the training data.** OpenAI's official docs (*Supervised fine-tuning*) state that GPT-4 fine-tuning carries an **input-dataset moderation** system that scans the training examples for harmful content and, on detection, **blocks the fine-tuning job**; GPT-4o / GPT-4o mini fine-tuning add **output moderation** on top of input moderation — after training, an **eval** system checks whether the resulting model's outputs violate the usage policy, and on a hit **blocks the job / withholds deployment**. Each harmful category has a **pass threshold**; if too many examples fail, it's blocked, and you can see which categories failed in the job's moderation-checks section or by querying the job events for `moderation_checks`-type events.
- **Failed runs aren't charged, and monitoring continues.** If a fine-tuning job fails because the training data is harmful, or the resulting model can't deploy because its outputs are harmful, **that training run isn't charged**; once live, OpenAI also **continuously runs automated safety evals on the fine-tuned model and monitors usage** to keep it within the usage policy.
- **But this gate does not catch alignment erosion (important).** Input moderation **only blocks clearly harmful examples**, and OpenAI itself acknowledges it is **not fully effective against data poisoning** — a few poisoned points slip under the blocking threshold. More to the point: it targets "harmful training content" and **does nothing about** the chain where benign fine-tuning wears away safety / privacy alignment as a side effect (Chain 2). **So "the vendor already moderated my fine-tuning data" ≠ "alignment didn't regress after fine-tuning"** — Qi et al. below is exactly the mechanism underneath this.

**How the industry does it, part 2 — vendors' data-boundary terms**: beyond the on-the-wire gate, vendors each have written terms on "fine-tuning-data training use / retention / fine-tuned-model ownership" — see the `:::caution` table at the end of this section (OpenAI / Anthropic / Google, verified cell by cell, stamped 2026-06, check current terms before deploying).

**Mechanism underneath — fine-tuning erodes alignment (Qi et al., ICLR 2024; research backing, not the headline)**: the reason that vendor moderation gate can't stop the alignment regression is, mechanistically, what Qi et al. already make clear —
...
| Anthropic | Official: commercial / API I/O not used for training by default (not trained on without your express permission) | Per its *API and data retention*: **as of 2025-09-14 standard API log retention cut from 30 to 7 days**, with a 30-day opt-in via DPA; **certain models still require 30-day retention**; under ZDR only what's needed for abuse screening is kept (including User Safety classifier results) | Under commercial terms the output is yours; specific fine-tuning / customization availability and terms are per its current product docs |
...
(The table is stamped 2026-06: the OpenAI row is per its *Fine-tuning guide* / *Supervised fine-tuning* and data-controls pages; the Anthropic row is per its *API and data retention* page; the Google row is per its *Vertex AI / Gemini data governance* page. Verify the current official docs and your contract before deploying.)
:::
...
Mixed evidence — **primary: Official docs** (OpenAI / Anthropic / Google fine-tuning and data-governance pages: how vendors guard fine-tuning on the wire + the data-boundary terms); **supplementary: Research** (Qi et al. ICLR'24, alignment erosion — as mechanism backing, explaining "why guarding still doesn't catch it").

- [OpenAI, Supervised fine-tuning guide (official)](https://platform.openai.com/docs/guides/supervised-fine-tuning) — official docs (this entry's industry-practice primary): GPT-4 fine-tuning runs **input moderation on the training data** and blocks the job on harmful content; GPT-4o / GPT-4o mini add an **output-moderation eval**, blocking / withholding deployment when the resulting model violates the usage policy (per-category thresholds; check `moderation_checks` events); failed runs aren't charged, and automated safety evals continue once live. Note: OpenAI acknowledges input moderation is **not fully effective against data poisoning**.
...
- [Anthropic, API and data retention (official)](https://platform.claude.com/docs/en/manage-claude/api-and-data-retention) — official docs: commercial / API inputs and outputs not used for training by default; as of 2025-09-14 standard log retention cut from 30 to 7 days (30-day opt-in via DPA), certain models still require 30-day, ZDR.
...
- [Fine-tuning Aligned Language Models Compromises Safety, Even When Users Do Not Intend To! (Qi et al., ICLR 2024)](https://openreview.net/forum?id=hTEGyKf0dZ) — mechanism backing (**safety-alignment erosion**, not data leak): on OpenAI's fine-tuning API, about 10 adversarial examples costing under $0.20 stripped GPT-3.5 Turbo's guardrails; benign data (Alpaca / Dolly) also unintentionally lowered safety on GPT-3.5 Turbo and Llama-2-7b-Chat. Used to explain "why even the vendor's moderation gate doesn't catch the alignment regression," not extrapolated to "fine-tuning-data extraction."
