...
This checklist is the seed of the toolkit's [LLM vendor data-boundary checklist](../vendor-data-boundary-checklist.mdx) — **use it as an artifact, file it per vendor, review it quarterly**, because the terms change.

**Minimal testable assertions** (treat the checklist as a regression artifact):

- How to test: keep a filled checklist per vendor, each cell carrying "source of the term + version date," reviewed quarterly.
- Pass: every cell has a source and a date and matches your signed contract / DPA; no blank cells, no stale cells.
- Fail: a cell has no source / contradicts the contract / is past its review date → mark it "unverified" and fill it in before any deployment decision.
...
Mixed evidence — **primary: Official docs** (OpenAI / Anthropic official pages); **supplementary: framework** (OWASP LLM02).

- [Data controls in the OpenAI platform (OpenAI official)](https://platform.openai.com/docs/guides/your-data) — Official docs: API not trained on by default, abuse-log retention, enterprise zero data retention (ZDR).
