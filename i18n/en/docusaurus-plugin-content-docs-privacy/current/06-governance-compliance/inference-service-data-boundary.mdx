...
flowchart LR
    ...
    T -->|most APIs default no<br/>consumer products often default yes| L["abuse-monitoring logs<br/>kept N days"]
...
- **Training use**: most APIs default to no training, but **consumer products often default to yes** (you must opt out) — don't conflate them.
...
:::caution The following are point-in-time vendor terms and change — verify the latest docs before citing
- **OpenAI (API)**: since 2023-03-01, API data is **not used for training by default** (unless you explicitly opt in); abuse-monitoring logs are kept by default for **about 30 days** then deleted (longer if legally required or to prevent abuse); **zero data retention (ZDR)** is offered to eligible enterprise customers, not a self-serve toggle — enabled per endpoint by your account team (OpenAI, *Data controls in the OpenAI platform*).
- **Anthropic (API)**: under commercial terms (Claude for Work / Enterprise / Education / Gov), **inputs and outputs are not used for training**; API log retention dropped **from 30 days to 7 days as of 2025-09-14**, auto-deleted at expiry; eligible enterprises can sign ZDR (Anthropic, *API and data retention*).
- **"Deletion" can be overridden by law**: in its litigation with The New York Times, OpenAI was at one point ordered by a court to **preserve** data it would otherwise delete — showing that a legal hold sits above any retention promise, and "deleted at expiry" is not absolute.
:::

These three confirm the same thing: **a data boundary is a set of terms that are checkable, changeable, and subject to outside law — not a line that says "we take privacy seriously."**
...
- **Consumer tier vs. API tier default differently.** At the same vendor, consumer products often default to training on your data (you must opt out), while APIs often default not to — don't carry one tier's assumptions to the other.
