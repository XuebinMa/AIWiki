...
> In one sentence: "Train on private data and it just becomes statistics — you can't get it back out" is a dangerous illusion. To an outside attacker, if a piece of text appeared in training at all (even in a single document, as long as it's rare and distinctive enough), I **may reproduce it verbatim under the right prompt** — and the **bigger the model, the more often it was duplicated, and the more context you give, the easier it comes out**.
...
3. Audit memorization before launch: inject canaries (fixed-format rare strings),
   measure their exposure afterward; abnormally high exposure -> go back and
   strengthen dedup / DP.
...
- **Risk scales with capability.** Bigger capacity means more memorization — scaling the model up for more power scales this privacy risk up with it.
...
Verbatim memorization and extractability are **mechanism-level phenomena of autoregressive language models** — common across vendors and versions, not one model's temperament. The attacks keep evolving over time: established on GPT-2 in 2021 (Carlini, USENIX), quantified in 2023 into log-linear relationships with scale / duplication / context (Carlini, ICLR), and demonstrated via divergence attack on production models like ChatGPT (Nasr). **Context windows and model sizes grow with each version, and memorization capacity grows with them** — bigger, more capable models make this risk heavier, not lighter. (Sources verified 2026-06.)
:::
