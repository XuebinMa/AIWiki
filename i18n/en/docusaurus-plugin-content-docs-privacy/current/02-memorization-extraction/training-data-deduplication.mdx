...
sources:
  ...
  - { title: "Penedo et al., The RefinedWeb Dataset for Falcon LLM (NeurIPS 2023 Datasets; arXiv 2306.01116) — a production-scale two-stage dedup pipeline (Falcon)", url: "https://arxiv.org/abs/2306.01116" }
---
...
(This entry's maturity is "Production": dedup is **standard pre-training data preparation** — mainstream production pretraining pipelines all include dedup; below is one public production pipeline, then the empirical evidence for its privacy gain.)

- **Production data pipelines really do dedup (one public example)**: RefinedWeb (Penedo et al., TII, NeurIPS 2023; the public dataset used to train Falcon-40B / 180B) runs **two stages of stringent dedup** on CommonCrawl (MinHash fuzzy dedup + exact dedup), distilling ~1 billion pages down to ~2.8 TB of unique text, and reports that stringent dedup consistently lifts downstream performance. This turns "dedup is a standard production step" from an assertion into a checkable public practice.
