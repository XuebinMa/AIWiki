...
- **Central route**: FL (raw data stays) + DP-FTRL (DP accounting suited to federated streaming, giving a user-level (ε, δ) bound) + secure aggregation (server sees only the sum). Drop any one: no DP → updates leak; no secure aggregation → the server sees single updates.
