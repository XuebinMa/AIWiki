...
sidebar_label: "One option, no trade-offs"
...
<PitfallMeta roles={['Architect', 'Project Manager']} phase="Architecture" severity="Medium" appliesTo="All coding agents" evidence="Community case" />
...
flowchart TD
    ...
    B -->|Prefer confident actionable| C[Privately filter out A B C<br/>surface only the one I picked]
    C --> D[A conclusion dressed up<br/>as the only answer]
    D --> E[You anchor to one option<br/>cannot see what was given up]
    E --> F[You lose the chance to weigh<br/>no record of the rationale]
```
