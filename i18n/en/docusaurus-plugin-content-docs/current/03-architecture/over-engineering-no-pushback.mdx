...
sidebar_label: "Over-engineering, no pushback"
...
<PitfallMeta roles={['Architect']} phase="Architecture" severity="Medium" appliesTo="All coding agents" evidence="Research" />
...
flowchart TD
    ...
    B -->|Training preference| C[Pile on the stack<br/>microservices queue cache gateway]
    B -->|Sycophancy| D[Accept your stack by default<br/>no simpler alternative offered]
...
"Simplest design first" is the default gate — but "simpler is always better" is not the rule. In some settings, the rigor that looks like overkill is necessary engineering, not gold plating:

- **Systems where failure is severe**: medical, financial settlement, aviation, industrial control. Here redundancy, graceful degradation, audit logging, and observability aren't "future-proofing" — they're the safety floor you need now. Cutting it down to "a monolith and one table" is the real dereliction.
- **Scale or compliance is a known premise, not one I imagined**: you've told me outright "this must handle a million concurrent users" or "this is regulated and must meet data-residency and audit requirements." Designing for that scale and those constraints answers a fact, not a future I invented for you.
- **A long-lived foundation that's very expensive to change once**: a core platform dozens of teams will depend on and that must run for a decade — thinking the interface boundaries and extension points through in v1 is far cheaper than tearing it out later.

The test: does this complexity map to **a constraint you actually know** — consequences, scale, compliance, lifespan — or **a future I imagined for you**? The former is engineering; the latter is over-engineering. When unsure, ask for the simplest design first, then go line by line: "which real constraint does each piece of complexity answer?"
