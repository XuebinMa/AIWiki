...
<PitfallMeta roles={['Architect', 'Engineer']} phase="Architecture" severity="Medium" appliesTo="All models" />
...
```mermaid
flowchart TD
    A[You raise a tiny need<br/>pad / deep-clone / format date<br/>&nbsp;] --> B{My default reach}
    B -->|reproduce frequent pattern| C[npm install a whole library]
    B -->|future-proofing| D[pre-add dependencies<br/>we might need<br/>&nbsp;]
    C --> E[bigger bundle, slower builds<br/>more transitive deps + attack surface<br/>&nbsp;]
    D --> E
    E --> F[upgrade, security, license<br/>bills land on you<br/>&nbsp;]
```
