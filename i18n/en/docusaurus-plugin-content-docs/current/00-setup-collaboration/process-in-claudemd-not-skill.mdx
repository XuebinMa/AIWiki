...
sidebar_label: "Process in CLAUDE.md, not a skill"
...
```mermaid
flowchart TD
    A[A chunk of content] --> B{Is it a fact I need every timeor a procedure for a certain task?}
    B -->|Fact/convention| C[CLAUDE.md<br/>resident every session]
    B -->|Multi-step procedure| D[skill<br/>loaded on demand, unfolds when used]
