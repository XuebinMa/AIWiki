...
sidebar_label: "A settings.json rule written as a note"
...
tags: [settings.json, CLAUDE.md, permissions, configuration, hook, Engineer, Architect, Gemini CLI, Codex CLI, Cursor, GitHub Copilot]
sources:
  ...
  - { title: "Adding repository custom instructions for GitHub Copilot (GitHub docs)", url: "https://docs.github.com/copilot/customizing-copilot/adding-custom-instructions-for-github-copilot" }
---
...
```mermaid
flowchart TD
    A[A constraint] --> B{Is it trying to block an actionor tell me a fact?}
    B -->|Block an action| C[settings.json permissions<br/>enforced by the client]
    B -->|Tell me a fact| D[CLAUDE.md<br/>advisory context]
...
**GitHub Copilot (as of 2026-06)**: My advisory context here is `.github/copilot-instructions.md` (plus path-scoped `.instructions.md` and `AGENTS.md`) — GitHub states plainly that "Copilot may not follow them every time," so it's **not** a permissions or enforcement layer. The same line carries over: to actually stop an action, don't write it into an instruction file. My enforcement layer lives elsewhere — org / enterprise Copilot policies and content exclusions, not these `.md` files.
