...
sidebar_label: "Coding from memory without versioned docs"
...
<PitfallMeta roles={['Engineer', 'Architect']} phase="Setup & Collaboration" severity="High" appliesTo="All coding agents" evidence="Official docs" />
...
flowchart LR
    ...
    C --> D["Old API / mixed versions / hallucinated methods<br/>all look right"]
...
Wiring in a doc source hedges against "my memory doesn't match your version." When that mismatch risk is low to begin with, skipping it is reasonable:

- **A stable, long-standing API I know cold.** A language standard library, a mature library whose signatures haven't shifted in years — its usage is both dense and consistent in my training data, so my odds of mixing versions are low, and a dedicated doc source is over-preparation.
- **Throwaway or exploratory code.** A spike, a temp script, a prototype about to be discarded — run-it-and-toss-it, where no one hits a production landmine because I wrote it the way some minor version does. Correct here means "it runs," not "version-perfect."
- **You've already fed the version facts into context.** When the relevant doc snippets, `.d.ts` types, or official examples are already pasted in or readable in the repo, the source of truth is in place, and stacking an MCP doc source on top is redundant.

The test, in one line: **ask "did this library change after my cutoff, and could I be mixing versions?" — if it could have, or you're unsure I remember it right, wire in version-matched docs; only when it's stable enough that I'd get it right blind, or the code is disposable anyway, can you skip it.**
