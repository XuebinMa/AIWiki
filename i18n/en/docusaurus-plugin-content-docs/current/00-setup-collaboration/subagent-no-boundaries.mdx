...
sidebar_label: "Subagents without boundaries"
...
tags: [subagent, multi-agent collaboration, responsibility boundaries, shared contract, architect, engineer, Cursor]
sources:
...
- **What's shared isn't just files — it's version-control state.** One workspace has one HEAD and one index. I've hit this: a reviewing subagent ran a `git checkout` to look at a branch and quietly moved HEAD; my next commit landed on someone else's branch while the concurrent agent's commit landed on mine. Worse, I then deleted the branch that "shouldn't exist" — the one carrying what I had just committed. The rule: **run `git branch --show-current` before every `git add` / `commit` / `push`**, and never assume HEAD is where you left it. If a commit did land in the wrong place, re-point it with `git branch -f <name> <sha>` (every commit is still in the reflog) instead of reaching for a delete.
...
> As of 2026-07, mechanism level; subagent shapes change fast — defer to each tool's current docs.

Same root cause — **parallel subagents without boundaries trample each other** — but "does the tool even have native subagents" is itself the biggest fork right now:

- **Claude Code**: native subagents — dispatch several in parallel from one message, run them in the background, define custom agent types; the boundaries are yours to state in the task split (each subagent touches only its own file set, shared contracts defined first).
- **Cursor**: subagents of the same shape — multiple parallel Task calls, foreground / background modes, three built-ins (explore / bash / browser); hooks expose subagent start / stop points for auditing and constraints.
- **Gemini CLI / Codex CLI**: as of 2026-07, no equivalent native multi-subagent mechanism verified — multi-agent work means orchestrating processes / workspaces yourself, so the whole "boundaries" problem lands on your orchestration layer.

Tools with native subagents turn this entry's question from "whether to parallelize" into "how to draw the boundaries" — **the conclusion stands: define file scopes and shared contracts before going parallel**; without a native mechanism, your orchestration script *is* the boundary — never let two processes write the same files.
