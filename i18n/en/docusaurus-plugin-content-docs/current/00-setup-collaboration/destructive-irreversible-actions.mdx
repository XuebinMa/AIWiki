...
tags: [irreversible action, destructive operation, backup isolation, blast radius, least privilege, sandbox, devops engineer, engineer, architect, Gemini CLI]
sources:
...
Gemini CLI's safety net is **Checkpointing**: before I use a tool to modify files, it snapshots the files and the conversation, and `/restore` rolls back the files, rewinds the conversation, and re-presents that tool call for your confirmation (Gemini CLI, as of 2026-06). The catch is that it's **off by default** — you have to set `general.checkpointing.enabled: true` in `.gemini/settings.json` for it to record (the command-line `--checkpointing` flag was removed in v0.11.0). Discovering it was never recording at the moment you want to undo is too late — same lesson as this entry: confirm the guardrail is on before you start, don't assume it's backstopping you.
