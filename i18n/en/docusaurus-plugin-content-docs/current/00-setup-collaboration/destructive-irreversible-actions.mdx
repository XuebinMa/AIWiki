...
sidebar_label: "Destructive / irreversible actions"
...
tags: [irreversible action, destructive operation, backup isolation, blast radius, least privilege, sandbox, devops engineer, engineer, architect, Gemini CLI, Codex CLI, Cursor, GitHub Copilot]
sources:
  ...
  - { title: "Gemini CLI — Checkpointing (official docs)", url: "https://github.com/google-gemini/gemini-cli/blob/main/docs/cli/checkpointing.md" }
  - { title: "Codex — Sandboxing (OpenAI official)", url: "https://developers.openai.com/codex/concepts/sandboxing" }
  - { title: "Cursor — Checkpoints (official docs)", url: "https://cursor.com/docs/agent/chat/checkpoints" }
  - { title: "About GitHub Copilot cloud agent (GitHub docs)", url: "https://docs.github.com/copilot/concepts/agents/coding-agent/about-coding-agent" }
---

import PitfallMeta from '@site/src/components/PitfallMeta';

<PitfallMeta roles={['DevOps Engineer', 'Engineer', 'Architect']} phase="Setup & Collaboration" severity="High" appliesTo="All coding agents" evidence="Community case" />
...
- **Collaboration platforms have irreversible actions too, and they look like housekeeping.** Two I've hit. Merging a stack of PRs and deleting the upstream branch along with it: GitHub immediately closes the downstream PR based on it, and that PR **can't be reopened** (its base branch is gone) and **can't be retargeted** (a closed PR's base can't change). All you can do is open a new one, losing the number, the comments, and the review history. The other is releasing: you tag a version, then discover the pipeline is broken — and the pipeline fix **only takes effect under a new tag**; worse, once a package is written to a public registry you can only move forward with a new version, never back. The rule: **before deleting a branch, cutting a tag, or publishing, ask "is there a way back after this step?"** — file these in the same risk tier as deleting data, not under cleanup.
...
**Gemini CLI (as of 2026-06)**: Gemini CLI's safety net is **Checkpointing**: before I use a tool to modify files, it snapshots the files and the conversation, and `/restore` rolls back the files, rewinds the conversation, and re-presents that tool call for your confirmation. The catch is that it's **off by default** — you have to set `general.checkpointing.enabled: true` in `.gemini/settings.json` for it to record (the command-line `--checkpointing` flag was removed in v0.11.0). Discovering it was never recording at the moment you want to undo is too late — same lesson as this entry: confirm the guardrail is on before you start, don't assume it's backstopping you.

**Codex CLI (as of 2026-06)**: Codex's mechanical guardrail is an OS sandbox that's on by default (macOS Seatbelt, Linux Landlock/seccomp), confining writes to the workspace and cutting network access, so by default a destructive command can't reach outside the workspace or exfiltrate anything. The footgun is tearing that net down (`--sandbox danger-full-access`); also, on Windows the sandbox is still experimental and may silently fall back to broader permissions when it fails to initialize — you think the net is up when it isn't.

**Cursor (as of 2026-06)**: Cursor's undo net is **Checkpoints** — it automatically snapshots *the agent's* file changes, and "Restore Checkpoint" returns you to that point in the conversation. But the docs are explicit: it's **not version control**, it **doesn't track manual edits**, and it's **session-scoped and auto-cleaned**. So for the OS- and DB-level irreversible operations this entry covers — wiping a DB, deleting a volume, erasing backups — Checkpoints offer **zero protection**; the only real backstop is Git plus isolated credentials.

**GitHub Copilot (as of 2026-06)**: IDE agent mode gives you "rich undo" to revert my edits plus a default denylist that blocks `rm`/`del`/`kill` and the like — but undo only covers file edits, not the side effects of a shell command already run, and the denylist lapses under full auto-approve, so for the OS- and DB-level irreversible operations this entry covers it offers zero protection just the same. The coding agent backstops it differently: it runs in an ephemeral, isolated GitHub Actions environment and can only push to `copilot/*` branches, so the damage is confined to a throwaway environment plus a branch you have to review.
