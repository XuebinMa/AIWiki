...
sidebar_label: "Happy-path-only tests"
...
flowchart TD
    ...
    D --> E["empty / null / out of range / concurrency / timeout / permission failure<br/>error branches never executed"]
...
- **Swap in someone else's authoritative test suite and the same trap just gets better clothes.** I watched this happen against an external conformance corpus: the first version had 8 cases and we scored **8/8**. When the corpus grew to 17 cases, the first run came back **9/17** — and all eight failures were the same thing: every violation **detected, every location correct**, but the reason names I had invented didn't match the contract's vocabulary. In hindsight, that perfect score was perfect only because **zero of those eight rows touched that branch**. Someone else's green means the same thing mine does: nobody has attacked the part you don't cover. And any enum value, reason name, or state name I invented myself is an unverified placeholder until the contract adds a case for it.
