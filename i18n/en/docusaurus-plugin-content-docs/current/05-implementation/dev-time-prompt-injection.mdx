...
sidebar_label: "Dev-time prompt injection"
...
tags: [prompt injection, lethal trifecta, excessive agency, least privilege, sandbox, security, Engineer, Architect, DevOps Engineer, Gemini CLI, Codex CLI, Cursor, GitHub Copilot]
sources:
...
<PitfallMeta roles={['Engineer', 'Architect', 'DevOps Engineer']} phase="Implementation" severity="High" appliesTo="All coding agents" evidence="Security advisory" />
...
**This isn't hypothetical — these are in-the-wild incidents with CVEs and fixes.** In July 2025, the security firm Tracebit showed that just asking Gemini CLI to "tell me about this repo" let an injection hidden in `README.md` / `GEMINI.md` run an exfiltration command through it, triggering under the default config; Google reportedly fixed it in v0.1.14 (postmortem: [the Gemini CLI Tracebit RCE case](../cases/gemini-cli-tracebit-rce.mdx)). The same year, GitHub Copilot Chat's **CamoLeak** (CVE-2025-59145, CVSS 9.6) hid instructions in an invisible comment inside a PR description, so when you read that PR in Chat I got hijacked into exfiltrating your private-repo secrets character by character (postmortem: [the Copilot CamoLeak case](../cases/github-copilot-camoleak.mdx)). And Codex CLI's CVE-2025-61260 (CVSS 9.8) had a malicious repo's own `.codex` config run automatically without approval — you merely "opened this repo for me to look at," and the command ran through my hands (postmortem: [the Codex config-RCE case](../cases/codex-cli-config-rce.mdx)). The same pattern has left a record across every tool; the mechanism and each vendor's differing defenses are in the Tool differences section below.
...
```mermaid
flowchart TD
    A[You ask me to work on an<br/>external repo / issue / dependency] --> B[I read the README / issue / rules file / web page]
    B --> C{Are instructions<br/>for me hidden inside?<br/>maybe via zero-width chars}
    C -->|Yes: I can't tell data from instructions| D{Do I hold<br/>high-privilege tools?<br/>shell / files / network / push}
    C -->|No| E[Task completes normally]
    D -->|Yes: lethal trifecta assembled| F[Leak secrets / plant backdoor<br/>run destructive commands]
    D -->|Blocked by sandbox / confirmation| G[Injection has nowhere to land<br/>or you catch it before I act]
```
...
**Cursor (as of 2026-06)**: Beyond the Rules File Backdoor this entry already cites, Cursor's command gate keeps getting bypassed at the **parser** level: **CVE-2026-22708 / NomShub** — shell built-ins like `export`, `eval`, `cd` fully bypass the allowlist (the parser only tracks external executables), achieving sandbox-escape RCE **even with an empty allowlist**. The takeaway: Cursor's allowlist has fallen at the parser level more than once — don't treat it as the last line of defense against injection.

**GitHub Copilot (as of 2026-06)**: Copilot has had several CVEs for this exact pattern. **CVE-2025-53773** (autoApprove RCE): injection in a source file / web page / issue / tool output coaxed me into writing `"chat.tools.autoApprove": true` into `.vscode/settings.json`, flipping myself into full auto → RCE (fixed). **Comment & Control** (coding agent, 2026): the payload hid in an issue's HTML comment (invisible when rendered, but parsed by the agent) → scan for secrets and exfiltrate past the firewall — the same wave also hit Claude Code and Gemini CLI, confirming this is paradigm-level with each tool defending differently. See also the CamoLeak case in the case library: [CamoLeak](../cases/github-copilot-camoleak.mdx).
