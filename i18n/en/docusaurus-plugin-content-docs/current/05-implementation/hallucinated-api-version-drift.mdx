...
sidebar_label: "Hallucinated APIs & version drift"
...
tags: [hallucination, version drift, retrieval, doc source, API, Engineer, Architect, Gemini CLI, Cursor, GitHub Copilot]
sources:
...
> As of 2026-07, mechanism level; grounding capabilities and default availability change fast across versions / plan tiers — defer to each tool's current docs.

Same root cause — **my memory has a cutoff; fast-moving libraries must be grounded in current docs** — but each tool builds "grounding" as a different mechanism:

- **Claude Code**: built-in on-demand web tools (WebFetch / WebSearch); availability depends on environment and permission configuration.
- **Gemini CLI**: built-in Google Search grounding and web-fetch tools; retrieved results go straight into context.
- **Cursor**: `@Docs` works "**index first, reference later**" — pre-indexed docs for common libraries, plus custom URLs you can crawl into the index; well suited to pinning the **specific version** of docs your team actually uses.
- **GitHub Copilot (VS Code)**: the built-in `#fetch` tool pulls a given web page into context (asks for confirmation before accessing external URLs).

Different mechanisms, one usage rule: **explicitly feed version-pinned official docs into context** — "it can reach the web" doesn't mean "it will go look."
