...
<PitfallMeta roles={['Engineer', 'DevOps Engineer']} phase="Implementation" severity="High" appliesTo="All Claude Code versions" evidence="Research" />
...
```mermaid
flowchart TD
    A["Hand me the whole task<br/>auto-approve, walk away<br/>&nbsp;"] --> B{"Can I reliably judge<br/>'stop / done'?<br/>&nbsp;"}
    B -- "No (my weak spot)" --> C["Optimistically try once more<br/>can't feel the mounting cost<br/>&nbsp;"]
    C --> D["Repeat the same action<br/>or ping-pong between two<br/>&nbsp;"]
    D -- "no external stop condition" --> C
    D --> E["Burn tokens / budget / rate<br/>write to external systems repeatedly<br/>&nbsp;"]
