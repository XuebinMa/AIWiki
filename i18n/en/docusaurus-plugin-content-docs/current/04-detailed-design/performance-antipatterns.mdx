...
<PitfallMeta roles={['Engineer']} phase="Detailed Design" severity="Medium" appliesTo="All models" evidence="Research" />
...
```mermaid
flowchart TD
    A[Your spec<br/>describes function only<br/>&nbsp;] --> B[I implement the most straightforward way]
    B --> C[Small-data tests all green]
    C --> D{You assume it's fine}
    D --> E[Real production scale]
    E --> F[N+1 / O n squared<br/>recompile inside loop<br/>blocking serial IO<br/>&nbsp;]
