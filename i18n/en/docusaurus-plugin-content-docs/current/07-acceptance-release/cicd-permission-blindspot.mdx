...
sidebar_label: "CI/CD permission blind spot"
...
tags: [CI/CD, permissions, least privilege, release, DevOps Engineer, Gemini CLI, Codex CLI, Cursor, GitHub Copilot]
sources:
  ...
  - { title: "run-gemini-cli security advisory GHSA-wpqr-6v78-jr5g (CVSS 10.0)", url: "https://github.com/google-github-actions/run-gemini-cli/security/advisories/GHSA-wpqr-6v78-jr5g" }
...
<PitfallMeta roles={['DevOps Engineer']} phase="Acceptance & Release" severity="High" appliesTo="All coding agents" evidence="Official docs" />
...
flowchart TD
    ...
    B --> C[Slip in: auto-enable a service<br/>auto-grant a permission]
    C --> D{Runtime permission boundary}
    D -->|Too little permission| E[Pipeline fails<br/>Resource not accessible]
    D -->|Permission gets widened| F[Attack surface grows<br/>least privilege violated]
```
...
- **The gate may not exist at all, while the interface makes it look like it does.** Three I've hit, all the same shape. (1) The "wait for checks before merging" switch waits only on the checks **marked required** — and that trunk had **no protection rules at all**, so the required set was empty and "auto-merge" meant merge immediately, landing on trunk while checks were still running. (2) The CLI shows both "hasn't started yet" and "finished but hasn't reported" as pending; the two look identical, and only the run's own job status tells them apart. (3) I believed a publish had used short-lived tokenless credentials because the job went green — what actually proves it is whether the artifact carries a provenance attestation, and the line in the log that looked like a masked token was the **literal placeholder** the tooling writes into its config when no token was supplied. The rule: **green only says some process finished; it doesn't say it took the path you assume. To confirm a gate, inspect the gate's own configuration, not how it renders in a UI.**
