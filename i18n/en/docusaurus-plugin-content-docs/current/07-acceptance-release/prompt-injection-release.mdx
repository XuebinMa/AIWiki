...
sidebar_label: "Prompt injection after launch"
...
<PitfallMeta roles={['Engineer', 'Architect', 'DevOps Engineer']} phase="Acceptance & Release" severity="High" appliesTo="All coding agents" evidence="Security advisory" />
...
```mermaid
flowchart LR
    A[Attacker hides instructions in<br/>user input / web page / email] -->|untrusted content enters context| B[Backend LLM reads it and is misled]
    B --> C{Can it directly drive<br/>sensitive tools?}
    C -->|Yes: input wired straight to tools| D[Oversteps: leaks data<br/>calls tools it shouldn't]
    C -->|Isolated / needs confirmation| E[Injection has nowhere to land<br/>or is stopped before it acts]
```
