...
sidebar_label: "\"The demo runs, ship it\""
...
<PitfallMeta roles={['Project Manager', 'Engineer', 'QA Engineer']} phase="Acceptance & Release" severity="High" appliesTo="All coding agents" evidence="Official docs" />
...
flowchart TD
    ...
    B --> C[screenshot + import succeeded<br/>I say: looks fine]
    C --> D{checked against predefined acceptance criteria}
    D -->|no, demo treated as done| E[real scale / concurrency / error paths<br/>performance / security / accessibility all blank]
    E --> F[breaks on real data after release<br/>the most expensive rework at acceptance]
```
...
"Only item-by-item acceptance counts as done" is aimed at features that **ship to real users and touch real data**. When the deliverable isn't a release at all, that bar shouldn't be forced onto it:

- **A purely internal, throwaway demo**: a one-off to show the team an idea or prove "this path is technically viable" — no external users, no production data, deleted once it's been seen. Its whole goal is "run it for you once," and here "the happy path demos" genuinely is its entire acceptance bar.
- **A probe-style feasibility check**: a minimal prototype built to answer "can this API connect / is this algorithm fast enough" — discarded the moment you have the answer. Making me verify it against large data volumes, concurrency, and permissions is running a release-grade check on code that will never go live.

The test: ask "**will this ever touch real users or real data?**" If yes — even an internal canary — run full item-by-item acceptance; the exception only applies to something that lives between you and me and is discarded after viewing. The instant someone says "nice demo, let's just ship it," it stops being a demo and the bar snaps back to item-by-item acceptance.
