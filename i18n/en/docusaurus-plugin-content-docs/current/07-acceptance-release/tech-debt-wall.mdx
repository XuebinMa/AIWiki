...
sidebar_label: "The tech-debt wall"
slug: tech-debt-wall
sidebar_position: 6
...
<PitfallMeta roles={['Architect', 'Engineer', 'Project Manager']} phase="Acceptance & Release" severity="High" appliesTo="All coding agents" evidence="Research" />
...
```mermaid
flowchart LR
    A[Just make it run now<br/>skip structure/tests/docs] --> B[First few weeks: output flies<br/>debt quietly compounds]
    B --> C[Hit the wall at some point<br/>change one, three break<br/>nobody dares touch it]
    C --> D[Speed falls off a cliff<br/>repay with interest]
```
