...
- **It isn't only logs — anything I generate that something downstream parses as structure is the same surface.** The most overlooked one is "a command for a human to copy." I assemble a hint saying "next, run: `tool --branch <branch>`", and that branch name comes from the repository, so an attacker can name it. Drop a semicolon and a command inside it, and the person's copy-paste executes it for them. What's worth remembering is where the bug came from: **it took shape in the very change that turned the hint from a rough description into something copy-pasteable** — the same edit that improved usability also promoted the injection from "needs the human to adapt it" to "works in one step." The rule: **don't stop at logs; sweep the whole class of "strings I generate that something else will parse or execute"** — commands, URLs, CSV rows, YAML/JSON fragments, commit messages. When an interpolation slot contains a metacharacter of that syntax, refuse to render and say why, rather than escaping and shipping it anyway.
