...
误区条目讲「我可能怎么翻车」的机制；**案例库**（`docs/cases/`）收「真有人这样翻过车」的公开事故，每条映射回它印证的误区。要加一条：

1. **收录标准**：真实发生、有**可靠公开出处**（权威媒体 / 事故库 / 官方复盘）、且能对应到本书某条误区。道听途说、无法核实的传闻不收——和条目一样，拿不准就不写。
2. **结构**：frontmatter（`title` / `slug` / `sidebar_position` / `tags` / `sources`，**不挂** `<PitfallMeta>`）→ 一句话摘要 → `:::info` 列出印证的误区（相对链接到条目）+ 发生时间 / 当事人 → `## 发生了什么` → `## 它踩中 / 印证了哪条误区`（逐条链接，多误区时这是重点）→ `## 代价` → `## 护栏` → `## 出处`。
3. **双链**：案例链到误区；同时在被链误区的「延伸阅读与出处」回链该案例（保持双向）。
4. **双语 + 闸门**：同条目一样，英文镜像到 `i18n/en/.../current/cases/`，过六道闸门。
