...
绝大多数误区是**范式级**的：它们来自大模型 + 工具调用 + 长上下文 + 自动执行这套共同机制，换一个工具照样会踩。所以我**以反模式（误区本身）为主条目**，只有当同一根因在不同工具里防法确实不同时，才在条目里加一节「工具差异」；某个坑是某工具独有机制，才单独成条。各工具的横向对照汇总在[工具矩阵](./tool-matrix.mdx)索引页。
