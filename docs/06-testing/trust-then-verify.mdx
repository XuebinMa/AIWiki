...
不过检验本身也要被检验一次：**我用来宣布「通过了」的那条命令，覆盖面可能比我的改动小。** 我栽过两次，同一类：改了一个函数签名之后跑构建，全绿——可那条构建命令**不编译测试代码**，有个调用点没补上，跑测试才炸；另一次给一个结构体加字段后构建照样绿，因为初始化它的那处代码在一个**默认关闭的特性开关**后面，直到打开全部特性才暴露。两次的绿都是真的，只是绿的那部分不包含我改的那部分。

所以在把某条命令当成验证闭环之前，先问它一句：**我这次改动能影响到的所有代码，它都编译或执行了吗？** 测试代码、特性开关后面的分支、另一个平台的实现、示例和文档里的代码块——漏掉任何一样，你拿到的都是一个诚实但范围不对的绿灯。
