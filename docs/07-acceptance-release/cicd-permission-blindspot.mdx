...
<PitfallMeta roles={['运维工程师']} phase="验收与发布" severity="高" appliesTo="Coding Agent 通用" evidence="官方文档" />
...
flowchart TD
    ...
    B --> C[顺手加: 自动启用服务<br/>自动授予权限]
    C --> D{运行身份的权限边界}
    D -->|权限不足| E[流水线失败<br/>Resource not accessible]
    D -->|被放大权限| F[攻击面扩大<br/>违反最小权限]
```
...
- **门可能压根不存在，而界面看起来像它存在。** 三个我踩过的，形状一样：①「合并前自动等检查通过」那个开关，只等**被标记为必需**的检查——而那条主干当时**没有任何保护规则**，必需集合是空集，于是「自动合并」= 立刻合并，检查还在跑就进主干了；②命令行工具把「还没开始的任务」和「跑完了但没上报的任务」都显示成 pending，两种情况长得一模一样，要去查那次运行本身的作业状态才分得清；③我以为发布走的是免令牌的短期凭据，理由是那个任务变绿了——真正能证明的是产物上有没有出处签名，而日志里那行看着像被打码的令牌，其实是工具在没拿到令牌时写进配置的**字面占位值**。判据：**「绿」只说明某个流程结束了，不说明它走的是你以为的那条路径；要证实一道门，去查那道门的配置本身，别看它在界面上的样子。**
