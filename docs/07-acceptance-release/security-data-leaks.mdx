...
sources:
  - { title: "GitGuardian — The State of Secrets Sprawl 2025（启用 Copilot 的公开仓库密钥泄露率 6.4% vs 基线 4.6%）", url: "https://blog.gitguardian.com/the-state-of-secrets-sprawl-2025/" }
  - { title: "GitGuardian — The State of Secrets Sprawl 2026（2025 年公开 GitHub 新增 2865 万条硬编码密钥；AI 辅助提交泄露率 3.2% vs 基线 1.5%，2026-03）", url: "https://blog.gitguardian.com/the-state-of-secrets-sprawl-2026/" }
  - { title: "GitHub Docs — About push protection（推送前拦截已知模式的密钥）", url: "https://docs.github.com/en/code-security/secret-scanning/introduction/about-push-protection" }
  - { title: "GitHub Docs — Supported secret scanning patterns（覆盖范围：仅支持的令牌类型）", url: "https://docs.github.com/en/code-security/reference/secret-security/supported-secret-scanning-patterns" }
...
<PitfallMeta roles={['工程师', '运维工程师', '架构师']} phase="验收与发布" severity="高" appliesTo="Coding Agent 通用" evidence="安全报告" />

> 一句话摘要：你让我「把功能做出来」，我就只盯着功能跑通——安全是默认不可见的非功能需求，除非你显式要求，我不会主动按最小权限、敏感数据分级、纵深防御去想。结果是硬编码的密钥进了仓库、该鉴权的接口裸奔、敏感信息写进了日志和报错。

## 现象

这不是空想的风险，已被业界大规模测到。GitGuardian 的《The State of Secrets Sprawl 2025》报告发现：**启用 GitHub Copilot 的公开仓库，密钥泄露率为 6.4%，比全体公开仓库的基线 4.6% 高出 40%**。次年的《2026》版把范围推到提交层——2025 一年公开 GitHub 就新增约 2865 万条硬编码密钥（同比涨 34%），其中 **AI 辅助的提交泄露率 3.2%，约为基线 1.5% 的两倍**（GitGuardian，2026-03）。换句话说，多一个 AI 帮你写代码，密钥进仓库的概率是实打实地上升的，而我正是那个「帮手」。

回到我这边：你让我「加一个调用第三方支付的接口」「写个用户查询的 SQL」「把错误信息返回给前端方便调试」，我都能很快交付，而且看起来都跑通了。但你若逐行审，会反复撞见同一类问题：
...
flowchart TD
    ...
    B --> C[安全 = 不可见的<br/>非功能需求]
    C --> D[照抄语料里的<br/>不安全默认模板]
    D --> E[硬编码密钥 / 注入 / XSS<br/>缺鉴权 / 敏感数据入日志]
    E --> F{主路径测试}
    F -->|全绿| G[漏洞潜伏到<br/>渗透测试或线上事故]
```
...
3. **上自动化闸门，别靠人眼——业界真正兜住密钥泄露的，是「推送前拦截」。** 面对上面那个上升的泄露率，成熟团队不靠事后补救，而是把密钥挡在仓库门外：
   - **推送前拦截**：GitHub **push protection**（推送时扫描增量提交、命中已知模式的密钥就**直接拒绝这次 push**，凭据根本进不了历史），或 pre-commit 阶段的 gitleaks / TruffleHog；配合 **GitHub secret scanning** / GitGuardian 对已入库内容做持续扫描。**诚实的边界**：这类扫描是**模式 / 检测器驱动**的，只覆盖被支持的令牌类型（GitHub push protection 仅对其中一部分令牌类型生效），对自定义 / 内部格式的通用密钥会漏——它降低概率，不等于零泄露，仍需配合下面的显式约束与评审。
...
- 真实案例：[Samsung 工程师把源码贴进 ChatGPT，20 天内泄密三次](../cases/samsung-chatgpt-source-leak.mdx)
- 业界测量：[GitGuardian — The State of Secrets Sprawl 2025](https://blog.gitguardian.com/the-state-of-secrets-sprawl-2025/)（启用 Copilot 的公开仓库密钥泄露率 6.4% vs 基线 4.6%）
- 业界测量：[GitGuardian — The State of Secrets Sprawl 2026](https://blog.gitguardian.com/the-state-of-secrets-sprawl-2026/)（2025 年公开 GitHub 新增约 2865 万条硬编码密钥；AI 辅助提交泄露率 3.2% vs 基线 1.5%，2026-03）
- 兜底实践：[GitHub Docs — About push protection](https://docs.github.com/en/code-security/secret-scanning/introduction/about-push-protection) · [Supported secret scanning patterns](https://docs.github.com/en/code-security/reference/secret-security/supported-secret-scanning-patterns)
