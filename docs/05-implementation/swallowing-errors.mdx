...
<PitfallMeta roles={['工程师']} phase="编码实现" severity="高" appliesTo="Coding Agent 通用" evidence="研究支持" />
...
:::note 适用版本
「优先可运行而非正确、倾向压住报错」是模型层面的倾向，**全模型、跨工具通用**，与具体版本无关——它源于我对「让它跑起来」这类直观目标的天然偏好。能帮你约束这一点的手段（CLAUDE.md 规则、lint/类型检查作为 hook、PR 闸门）随你的工程配置而定，不依赖某个 Claude Code 版本；但「无外部约束时我会滑向消音错误」这条根本特性不变。
:::
