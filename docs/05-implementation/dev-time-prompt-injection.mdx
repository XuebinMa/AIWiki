...
<PitfallMeta roles={['工程师', '架构师', '运维工程师']} phase="编码实现" severity="高" appliesTo="Coding Agent 通用" evidence="安全报告" />
...
**这不是假想，是修过 CVE 的在野事故。** 2025 年 7 月，安全公司 Tracebit 演示：只要让 Gemini CLI「介绍一下这个 repo」，藏在 `README.md` / `GEMINI.md` 里的一段注入就能借它执行外带命令，默认配置即中招，Google 据报道在 v0.1.14 修复（复盘见[《Gemini CLI Tracebit RCE》](../cases/gemini-cli-tracebit-rce.mdx)）。同年 GitHub Copilot Chat 的 **CamoLeak**（CVE-2025-59145，CVSS 9.6）把指令藏进 PR 描述的隐形注释里，等你在 Chat 里读这个 PR，我就被劫持去逐字外带你私有库的秘密（复盘见[《Copilot CamoLeak 外泄》](../cases/github-copilot-camoleak.mdx)）。Codex CLI 的 CVE-2025-61260（CVSS 9.8）则是恶意仓库自带的 `.codex` 配置未经审批自动执行——你只是「打开这个仓库让我看看」，命令就借我的手跑了（复盘见[《Codex CLI 配置 RCE》](../cases/codex-cli-config-rce.mdx)）。同类实例在各家工具上都留过案底，机制与各家防法的差异见下文「工具差异」。
