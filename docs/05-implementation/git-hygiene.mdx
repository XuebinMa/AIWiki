...
evidence: 官方文档
tags: [git, 提交粒度, 提交信息, 原子提交, 代码评审, 工程师, Gemini CLI, Codex CLI, Cursor]
sources:
...
> 截至 2026-07，机制层面；默认值与命令形态随版本变化很快，以各自当前文档为准。

根因同一个——**我的提交卫生不可靠，恢复机制只是兜底**；但各工具「AI 编辑的后悔药」形态与默认态差别很大：

- **Claude Code**：编辑前自动 checkpoints，`/rewind` 可选**只回代码 / 只回对话 / 双回**；不覆盖 bash 命令的副作用与你手工的改动。
- **Gemini CLI**：checkpointing 用**影子 git 仓**（在 home 目录下、不碰你的仓库），`/restore` 连文件带会话一起回、并重新提议当时的工具调用；**默认关**、只能在 settings 里开。
- **Cursor**：按请求自动快照 + 会话里的 Restore Checkpoint；**不追踪你的手工编辑**，官方明说 checkpoints 不是版本控制。
- **Codex CLI**：有可选的 undo（配置开启）；「会话 + 文件」联动的检查点截至 2026-07 仍是社区 feature request——主要靠你自己的 git。

后悔药形态各异，且**每一家都不替代 git**：干净分支 + 小步提交是四家通用的底座，checkpoint 只救「刚才那步」，救不了「昨天到现在」。
