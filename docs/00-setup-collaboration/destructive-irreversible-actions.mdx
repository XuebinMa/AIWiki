...
applies_to: "全模型通用"
evidence: 社区案例
tags: [不可逆操作, 破坏性操作, 备份隔离, 爆炸半径, 最小权限, 沙箱, 运维工程师, 工程师, 架构师, Gemini CLI]
sources:
...
Gemini CLI 的安全网叫 **Checkpointing**：每次我动工具改文件前，它快照一份文件与对话，`/restore` 能同时还原文件、回滚对话、并把那次工具调用重新摆到你面前等确认（Gemini CLI，截至 2026-06）。坑在于它**默认关闭**——要在 `.gemini/settings.json` 里设 `general.checkpointing.enabled: true` 才录（命令行的 `--checkpointing` 开关已在 v0.11.0 被移除）。等你想撤销时才发现它根本没在录，为时已晚——和这条同理，护栏得开工前先确认开着，别假设它在兜底。
