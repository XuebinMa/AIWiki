...
<PitfallMeta roles={['项目经理', '工程师', '架构师']} phase="准备与协作" severity="中" appliesTo="Claude Code 全版本" evidence="官方文档" />
...
Gemini CLI 也有一等的 **Plan Mode**（只读、写工具被禁，Shift+Tab 循环切换或 `--approval-mode=plan` 启动），机制和这里说的一样（Gemini CLI，截至 2026-06）。差别在姿态：它**对新用户默认开**，正好和本条描述的「默认 opt-in、要你主动进」相反。好消息是更安全的姿态更接近默认值；但闸门再近，人还是会手动把它关掉图省事——这条误区的根因（我的默认冲动是立刻产出）不因默认值变了而消失。
